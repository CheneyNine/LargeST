...
class E2CSTP_Engine(BaseEngine):
    def __init__(self, branch_alpha, causal_reg_weight, **args):
        super(E2CSTP_Engine, self).__init__(**args)
        self._branch_alpha = branch_alpha
        self._causal_reg_weight = causal_reg_weight
    ...
    def train_batch(self):
        ...
        for X, label in self._dataloader["train_loader"].get_iterator():
            ...
            main_pred = outputs["main_prediction"]
            aux_pred = outputs["aux_prediction"]
            causal_penalty = outputs["causal_penalty"]

            pred, main_pred, aux_pred, label = self._inverse_transform(
                [pred, main_pred, aux_pred, label]
            )
            ...
            final_loss = self._loss_fn(pred, label, mask_value)
            main_loss = self._loss_fn(main_pred, label, mask_value)
            aux_loss = self._loss_fn(aux_pred, label, mask_value)
            loss = (
                final_loss
                + self._branch_alpha * main_loss
                + (1.0 - self._branch_alpha) * aux_loss
                + self._causal_reg_weight * causal_penalty
            )
